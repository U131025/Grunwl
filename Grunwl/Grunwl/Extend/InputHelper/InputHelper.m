...
#define INT_NUMBER_POINT_SET @"^\\d+(\\.\\d{1,2})?$"
...
- (void)setupValidationType:(ValidationType)type forInputField:(UIView *)inputField {
    ...
    switch (type) {
        case ValidationTypeNumberInt:
        {
            ...
            break;
        case ValidationTypeNumberPoint:
        {
            [self setKeyBoardType:UIKeyboardTypeDecimalPad forInputField:inputField];
        }
            break;
...
//        CGFloat offsetY = IS_IOS7_OR_LATER ? 64.0f : 0;
        _currentRootView.frame = (CGRect){frame.origin.x, 0, frame.size.width, frame.size.height};
    }];
...
- (void)checkInputField:(UIView *)inputField{
    ...
    if (text.length) {
        ...
        switch (type) {
            ...
            case ValidationTypeNumberPoint:
            {
//                [self checkText:text forInputField:inputField validation:INT_NUMBER_POINT_SET];
                [self setText:[text trimmedString] forInputField:inputField];
            }
                break;
